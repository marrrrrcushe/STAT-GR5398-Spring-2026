...
def prepare_rolling_train(df,features_column,label_column,date_column,unique_datetime,testing_windows,first_trade_date_index, max_rolling_window_index,current_index):
    ...
    report_date = train["reportdate"].values
    return X_train,y_train, report_date

def prepare_rolling_test(df,features_column,label_column,date_column,unique_datetime,testing_windows,first_trade_date_index, current_index):
    ...
    report_date = test["reportdate"].values
    return X_test,y_test, report_date

def prepare_trade_data(df,features_column,label_column,date_column,tic_column,unique_datetime,testing_windows,first_trade_date_index, current_index):
    ...
    report_date = trade["reportdate"].values
    return X_trade,y_trade,trade_tic, report_date
...
def append_return_table(df_predict, unique_datetime, y_trade_return, trade_tic, report_dates, current_index):
    ...
    for i in range(tmp_table.shape[1]):
        tic = tmp_table.columns[i]
        df_predict.at[pd.to_datetime(report_dates[i]), tic] = tmp_table[tic].iloc[0]
...
def run_4model(df,features_column, label_column,date_column,tic_column,
    ...
    df_predict_rf = pd.DataFrame(columns=unique_ticker)
    df_predict_gbm = pd.DataFrame(columns=unique_ticker)
    df_predict_xgb = pd.DataFrame(columns=unique_ticker)
    df_predict_best = pd.DataFrame(columns=unique_ticker)
    ...
    for i in range(first_trade_date_index, len(unique_datetime)):
        try:
            ...
            X_train, y_train, _ = prepare_rolling_train(df, 
                                                     features_column,
                                                     label_column,
                                                     date_column, 
                                                     unique_datetime, 
            ...
            X_test, y_test, _ = prepare_rolling_test(df, 
                                                  features_column,
                                                  label_column,
                                                  date_column, 
                                                  unique_datetime, 
            ...
            # prepare trade data
            X_trade, y_trade, trade_tic, report_dates = prepare_trade_data(df,
                                                             features_column,
                                                             label_column,
                                                             date_column,
                                                             tic_column, 
         ...
         #   lr_model = train_linear_regression(X_train, y_train)
            ...
            print(f"xgb:{time.perf_counter() - t:.6f}s")
            t = time.perf_counter()
            gbm_model = train_lightgbm(X_train, y_train)
            print(f"gbm:{time.perf_counter() - t:.6f}s")
            t =time.perf_counter()
            rf_model = train_random_forest(X_train, y_train)
            print(f"rf:{time.perf_counter() - t:.6f}s")
         #   ridge_model = train_ridge(X_train, y_train)
            ...
            append_return_table(df_predict_rf, unique_datetime, y_trade_rf, trade_tic, report_dates, current_index=i)
            append_return_table(df_predict_xgb, unique_datetime, y_trade_xgb, trade_tic, report_dates, current_index=i)
            append_return_table(df_predict_gbm, unique_datetime, y_trade_gbm, trade_tic, report_dates, current_index=i)
            append_return_table(df_predict_best, unique_datetime, y_trade_best, trade_tic, report_dates, current_index=i)
